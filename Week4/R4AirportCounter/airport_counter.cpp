...
public:
  ...
  size_t Get(TAirport airport) const {
      return _aps_departures[static_cast<uint32_t>(airport)].second;
  }

  // добавить данный элемент
  void Insert(TAirport airport) {
      _aps_departures[static_cast<uint32_t>(airport)].second++;
  };
  ...
  void EraseAll(TAirport airport) {
      _aps_departures[static_cast<uint32_t>(airport)].second = 0;
  }
  ...
  Items GetItems() const {
      return _aps_departures;
  };

private:
    Items _aps_departures;
    void _createArrayFromEnum(){
        size_t vector_count = static_cast<uint32_t>(TAirport::Last_);
        for (size_t i = 0; i < vector_count; i++) {
            _aps_departures[i] = { static_cast<TAirport>(i), 0 };
        }
    };
};
